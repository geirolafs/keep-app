...
| Styling | Tailwind CSS + shadcn/ui + Base UI | design system + headless primitives |
...
- [x] Rust: generate 400px JPEG thumbnail on save (image crate, FilterType::Triangle)
...
```
save_image(path | url | bytes) -> ImageRecord
delete_image(id)
get_images(filter, sort, page) -> Vec<ImageRecord>
search_images(query) -> Vec<ImageRecord>
add_tag(image_id, tag_name)
...
1. ~~App name / brand?~~ **KEEP** — bundle ID: `is.geir.mood`
